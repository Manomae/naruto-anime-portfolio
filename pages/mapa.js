...
export default function MapaSpatialChakraEmanuel() {
  ...
  const estabelecimentos = [
    { id: 1, nome: 'Emanuel.OS Core Data Center 01', categoria: '🖥️ Servidor de Dados & Nuvem AGI', cor: 0x00f0ff, posicao: { x: -6, y: 3, z: -4 }, ipCriptografado: 'AES256-88F9-90A1-EMA' },
    { id: 2, nome: 'Arena Futebol Clube & Parque Neon', categoria: '⚽ Estádio & Lazer Comunitário', cor: 0x00ff66, posicao: { x: 6, y: 1.5, z: 4 }, ipCriptografado: 'AES256-11B4-33C8-ARENA' },
    { id: 3, nome: 'Hospital Geral & Clínica Vet 24h', categoria: '🏥 Saúde Humana & Animal', cor: 0xff0055, posicao: { x: -5, y: 2.2, z: 5 }, ipCriptografado: 'AES256-55H2-88X9-HOSP' },
    { id: 4, nome: 'Usina Solar & Matriz Energética 3D', categoria: '⚡ Energia Limpa & Renovável', cor: 0xffaa00, posicao: { x: 7, y: 2, z: -6 }, ipCriptografado: 'AES256-99A1-11Z2-SOLAR' },
    { id: 5, nome: 'Fazenda Tech & Agronegócio Inteligente', categoria: '🌾 Agricultura Digital & Drones', cor: 0x88ff00, posicao: { x: -8, y: 1.2, z: -8 }, ipCriptografado: 'AES256-33D4-77E8-AGRO' },
    { id: 6, nome: 'Catedral & Templo de Luz', categoria: 'Igreja & Apoio Espiritual', cor: 0xaa00ff, posicao: { x: 0, y: 4, z: -7 }, ipCriptografado: 'AES256-77F8-44K1-TEMPLO' },
    { id: 7, nome: 'Praça Central & Quiosque Digital', categoria: '🌳 Convivência & Wi-Fi Livre', cor: 0x00aaff, posicao: { x: 0, y: 0.8, z: 2 }, ipCriptografado: 'AES256-22M9-11L3-PRACA' },
    { id: 8, nome: 'Centro Comercial & Shopping Cyber', categoria: '🛍️ Lojas & Mercado Digital', cor: 0xff00aa, posicao: { x: 4, y: 2.8, z: -1 }, ipCriptografado: 'AES256-44J3-99P8-LOJAS' }
  ];
  ...
  useEffect(() => {
    ...
    // 2. CONSTRUÇÕES, LINHAS DE CHAKRA E NUVENS DE IA
    estabelecimentos.forEach((est) => {
      ...
      // 🏃 PARTÍCULAS EM MOVIMENTO (Corrigido)
  ...
  return (
    <div style={{ width: '100vw', height: '100vh', backgroundColor: '#020204', overflow: 'hidden', position: 'relative', fontFamily: '"Segoe UI", sans-serif' }}>
      <Head>
        <title>Emanuel.OS - Linhas de Chakra & Monitoramento Multimodal 3D</title>
      </Head>
      ...
      <header style={{ position: 'absolute', top: '25px', left: '30px', zIndex: 10 }}>
        <h1 style={{ fontSize: '22px', margin: 0, color: '#fff', fontWeight: '900', letterSpacing: '1px' }}>
          🕸️ REDE DE CHAKRA <span style={{ color: '#00f0ff' }}>INTEGRADA 3D</span>
        </h1>
        <span style={{ fontSize: '11px', color: '#a1a1aa', fontWeight: 'bold' }}>
          Modelo Kankuro: Rastreamento Cognitivo de Fluxos e Tarefas Ativas da IA
        </span>
      </header>
      ...
      {localSelecionado && (
        <aside style={{ position: 'absolute', right: '30px', bottom: '30px', width: '380px', backgroundColor: 'rgba(7, 12, 28, 0.95)', border: '1px solid rgba(0, 240, 255, 0.5)', borderRadius: '20px', padding: '22px', backdropFilter: 'blur(25px)', boxShadow: '0 0 50px rgba(0, 240, 255, 0.3)', zIndex: 20 }}>
          ...
          <div style={{ backgroundColor: 'rgba(0,0,0,0.5)', padding: '8px 12px', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.05)', marginBottom: '15px' }}>
            <span style={{ fontSize: '10px', color: '#ff0055', fontWeight: 'bold', display: 'block' }}>🔒 CAPTAÇÃO DE NOVAS TAREFAS / INVESTIMENTOS (MONITOR FICTÍCIO)</span>
            <span style={{ fontSize: '10px', color: '#71717a', fontFamily: 'monospace' }}>LINK SECURE NODE: {localSelecionado.ipCriptografado}</span>
          </div>
